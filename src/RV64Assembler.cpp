...
void CRV64Assembler::ResolveLiteralReferences()
{
    ...
    for(const auto& literalRef : m_literal128Refs)
    {
        ...
        //uint32 o = m_stream->Read32();
        //auto rd = static_cast<REGISTER64>((o >> 7) & 0x1f);
        //m_stream->Seek(literalRef.offset, Framework::STREAM_SEEK_SET);
        auto rd = static_cast<REGISTER64>(literalRef.rtmp);
        if (offset & 0x800) {
            // 12th bit set
            int32 signedLower12 = SIGN_EXTEND_12(offset);
            Auipc(rd, ((offset-signedLower12) & 0xfffff000));
            Addi(rd, rd, signedLower12);
        } else {
            Auipc(rd, (offset & 0xFFFFF000));
            Addi(rd, rd, (offset & 0xFFF));
        }

        Vlwv(static_cast<REGISTERMD>(literalRef.rt), rd, 0);
        //Break();
    }
...
{
    if (m_thead_extentions) {
        Vaddvi(rd, v0, 0, 0); // save v0
        Vmseqvv(v0, rn, rm, 0);
        Vrsubvx(rm, v0, xZR, 0); // 0xffffffff or 0xfffffffe
        Vorvv(rn, rm, v0, 1); // masked with v0 now 0xffffffff or 0x00000000
        Vaddvi(v0, rd, 0, 0); // restore v0
        Vaddvi(rd, rm, 0, 0); // set rd
        return;
    }
...
void CRV64Assembler::Cmeq_4s(REGISTERMD rd, REGISTERMD rn, REGISTERMD rm)
{
    if (m_thead_extentions) {
        //Vaddvi(rd, v0, 0, 0); // save v0
        Vmseqvv(v0, rn, rm, 0);
        Vrsubvx(rd, v0, xZR, 0); // 0xffffffff or 0xfffffffe
        Vorvv(rd, rd, v0, 1); // masked with v0 now 0xffffffff or 0x00000000
        //Vaddvi(v0, rd, 0, 0); // restore v0
        //Vaddvi(rd, rm, 0, 0); // set rd
        //Break();
        return;
...
void CRV64Assembler::Ldr_Pc(REGISTERMD rt, const LITERAL128& literal, REGISTER64 rtmp)
{
    LITERAL128REF literalRef;
    literalRef.offset = static_cast<size_t>(m_stream->Tell());
    literalRef.value = literal;
    literalRef.rt = rt;
    literalRef.rtmp = rtmp;
...
void CRV64Assembler::Umin_4s(REGISTERMD rd, REGISTERMD rn, REGISTERMD rm)
{
    ...
    /*Vmvsx(v0, static_cast<REGISTER64>(rn));
    Vminuvx(v0, v0, static_cast<REGISTER64>(rm), 0);
    Vextxv(static_cast<REGISTER64>(rd), v0, x0);*/
...
void CRV64Assembler::Sw(REGISTER64 rs1, REGISTER32 rs2, int32 imm)
{
...
void CRV64Assembler::Smin_1s_RVV(REGISTER32 rd, REGISTER32 rn, REGISTER32 rm)
{
    /*for (int i=0; i<32; i++) {
        Addiw(rd, zero, i);
        Vmvsx(static_cast<REGISTERMD>(i), static_cast<REGISTER64>(rd));
    }
    Break();*/
    Vsetvli(xZR, xZR, 8);
    Vmvsx(v0, static_cast<REGISTER64>(rn));
    Vminvx(v0, v0, static_cast<REGISTER64>(rm), 0);
    Vextxv(static_cast<REGISTER64>(rd), v0, x0);
    Addiw(rd, rd, 0);
}

void CRV64Assembler::Umin_1s_RVV(REGISTER32 rd, REGISTER32 rn, REGISTER32 rm)
{
    Vsetvli(xZR, xZR, 8);
    Vmvsx(v0, static_cast<REGISTER64>(rn));
    Vminuvx(v0, v0, static_cast<REGISTER64>(rm), 0);
...
/*
 * R Format Atomic Instructions:
 */

// RV32A
void CRV64Assembler::Amominw(REGISTER64 rd, REGISTER64 rs1, REGISTER64 rs2)
{
    WriteR(0x8000202F, rd, rs1, rs2);
}

void CRV64Assembler::Min(REGISTER64 rd, REGISTER64 rs1, REGISTER64 rs2)
{
    WriteR(0x0A004033, rd, rs1, rs2);
}



void CRV64Assembler::Vlswv(REGISTERMD vd, REGISTER64 rs1, REGISTER64 rs2, int vm2) {
    uint32 opcode = 0x00000007;
    //vm = 0x120;
    uint32 width = 6; // vector word
    uint32 lumop_rs2_vs2 = rs2;
    uint32 vm = 1; // unmasked
    uint32 mop = 6; // sign-extended strided
    uint32 nf = 0;
...
void CRV64Assembler::Vlwv(REGISTERMD vd, REGISTER64 rs1, int vm2) {
    uint32 opcode = 0x00000007;
    //vm = 0x120;
    uint32 width = 6; // vector word
...
void CRV64Assembler::Vswv(REGISTERMD vs3, REGISTER64 rs1, int vm) {
    uint32 opcode = 0x02006027;
    opcode |= (vs3 << 7);
    opcode |= (rs1 << 15);
    opcode |= (vm << 20);
    WriteWord(opcode);
...
void CRV64Assembler::Vlev(REGISTERMD vd, REGISTER64 rs1, int vm2) {
    uint32 width = VWidth::VectorElement;
    uint32 lumop_rs2_vs2 = 0;
    ...
    uint32 nf = 0;
    Vloadv(vd, rs1, width, lumop_rs2_vs2, vm, lmop, nf);
}

void CRV64Assembler::Vsev(REGISTERMD vs3, REGISTER64 rs1, int vm2) {
    uint32 width = VWidth::VectorElement;
    uint32 lumop_rs2_vs2 = 0;
    uint32 sumop_rs2_vs2 = 0;
    uint32 vm = 1; // unmasked
    ...
    uint32 smop = 0; // unit-stride
    uint32 nf = 0;
    Vstorev(vs3, rs1, width, sumop_rs2_vs2, vm, smop, nf);
}
...
void CRV64Assembler::Vsetvli(REGISTER64 rd, REGISTER64 rs1, int vtypei) {
    ...
    opcode |= (vtypei << 20);
    WriteWord(opcode);
    //WriteWord(0x00857057);
}

void CRV64Assembler::Vextxv(REGISTER64 rd, REGISTERMD vs1, REGISTER64 rs1) {
    // vext.x.v a1, v0, x0
    uint32 opcode = 0x32002057;
    opcode |= (rd << 7);
    opcode |= (vs1 << 20);
    opcode |= (rs1 << 15);
    WriteWord(opcode);
    //WriteWord(0x320025d7);
}

void CRV64Assembler::Vmvsx(REGISTERMD vd, REGISTER64 rs1) {
    // vmv.s.x v0, a1
    uint32 opcode = 0x36006057;
    opcode |= (vd << 7);
    opcode |= (rs1 << 15);
    WriteWord(opcode);
...
void CRV64Assembler::Vminvx(REGISTERMD vd, REGISTERMD vs2, REGISTER64 rs1, int vm) {
    // vmin.vx v0, v0, a2
    uint32 opcode = 0x16004057;
    opcode |= (vd << 7);
    opcode |= (vs2 << 20);
    opcode |= (rs1 << 15);
    WriteWord(opcode);
    //WriteWord(0x16064057);
}

void CRV64Assembler::Vminuvv(REGISTERMD vd, REGISTERMD vs2, REGISTERMD vs1, int vm) {
    uint32 opcode = 0x12000057;
    opcode |= (vd << 7);
    opcode |= (vs2 << 20);
    opcode |= (vs1 << 15);
    WriteWord(opcode);
...
void CRV64Assembler::Vminuvx(REGISTERMD vd, REGISTERMD vs2, REGISTER64 rs1, int vm) {
    uint32 opcode = 0x12004057;
    opcode |= (vd << 7);
    opcode |= (vs2 << 20);
    opcode |= (rs1 << 15);
    WriteWord(opcode);
...
void CRV64Assembler::Vrsubvx(REGISTERMD vd, REGISTERMD vs2, REGISTER64 rs1, int vm) {
    uint32 opcode = 0x0e004057;
    opcode |= (vd << 7);
    opcode |= (vs2 << 20);
    opcode |= (rs1 << 15);
    WriteWord(opcode);
...
void CRV64Assembler::Vrsubvi(REGISTERMD vd, REGISTERMD vs2, int imm, int vm) {
    uint32 opcode = 0x0e003057;
    opcode |= (vd << 7);
    opcode |= (vs2 << 20);
    opcode |= (imm << 15);
    WriteWord(opcode);
...
void CRV64Assembler::Vsllvx(REGISTERMD vd, REGISTERMD vs2, REGISTER64 rs1, int vm) {
    uint32 opcode = 0x96004057;
    opcode |= (vd << 7);
    opcode |= (rs1 << 15);
    opcode |= (vs2 << 20);
    WriteWord(opcode);
...
void CRV64Assembler::Vsrlvx(REGISTERMD vd, REGISTERMD vs2, REGISTER64 rs1, int vm) {
    uint32 opcode = 0xa2004057;
    opcode |= (vd << 7);
    opcode |= (rs1 << 15);
    opcode |= (vs2 << 20);
    WriteWord(opcode);
...
void CRV64Assembler::Vsrlvv(REGISTERMD vd, REGISTERMD vs2, REGISTERMD vs1, int vm) {
    uint32 opcode = 0xa2000057;
    opcode |= (vd << 7);
    opcode |= (vs1 << 15);
    opcode |= (vs2 << 20);
    WriteWord(opcode);
...
void CRV64Assembler::Vandvi(REGISTERMD vd, REGISTERMD vs2, int imm, int vm) {
    uint32 opcode = 0x26003057;
    opcode |= (vd << 7);
    opcode |= (vs2 << 20);
    opcode |= ((imm & 0x1f) << 15);
    WriteWord(opcode);
...
void CRV64Assembler::Vslide1upvx(REGISTERMD vd, REGISTERMD vs2, REGISTER64 rs1, int vm) {
    uint32 opcode = 0x3a006057;
    opcode |= (vd << 7);
    opcode |= (vs2 << 20);
    opcode |= (rs1 << 15);
    WriteWord(opcode);
...
void CRV64Assembler::Vslide1downvx(REGISTERMD vd, REGISTERMD vs2, REGISTER64 rs1, int vm) {
    uint32 opcode = 0x3e006057;
    opcode |= (vd << 7);
    opcode |= (vs2 << 20);
    opcode |= (rs1 << 15);
    WriteWord(opcode);
...
void CRV64Assembler::Vmvvi(REGISTERMD vd, int imm, int vm) {
    uint32 opcode = 0x5e003057;
    opcode |= (vd << 7);
    opcode |= (imm << 15);
    WriteWord(opcode);
}

void CRV64Assembler::Vmvvv(REGISTERMD vd, REGISTERMD vs1, int vm) {
    uint32 opcode = 0x5e000057;
    opcode |= (vd << 7);
    opcode |= (vs1 << 15);
    WriteWord(opcode);
}

void CRV64Assembler::Vmvvx(REGISTERMD vd, REGISTER64 rs1, int vm) {
    uint32 opcode = 0x5e004057;
    opcode |= (vd << 7);
    opcode |= (rs1 << 15);
    WriteWord(opcode);
}

void CRV64Assembler::Vmergevvm(REGISTERMD vd, REGISTERMD vs2, REGISTERMD vs1, int vm) {
    uint32 opcode = 0x5c000057;
    opcode |= (vd << 7);
    opcode |= (vs2 << 20);
    opcode |= (vs1 << 15);
    WriteWord(opcode);
...
void CRV64Assembler::Vwaddvx(REGISTERMD vd, REGISTERMD vs2, REGISTER64 rs1, int vm) {
    uint32 opcode = 0xc6006057;
    opcode |= (vd << 7);
    opcode |= (vs2 << 20);
    opcode |= (rs1 << 15);
    WriteWord(opcode);
...
void CRV64Assembler::Vwadduvx(REGISTERMD vd, REGISTERMD vs2, REGISTER64 rs1, int vm) {
    uint32 opcode = 0xc2006057;
    opcode |= (vd << 7);
    opcode |= (vs2 << 20);
    opcode |= (rs1 << 15);
    WriteWord(opcode);
